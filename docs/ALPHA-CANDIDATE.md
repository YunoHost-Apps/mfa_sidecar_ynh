...
- LDAP behavior is host-shaped now, but still needs confirmation under real login / enrollment flows on target
- LDAP bind password is still seeded as placeholder (`CHANGEME_LDAP_BIND_PASSWORD`) and must be replaced during real deployment
- services currently run as root in draft units to minimize packaging friction; least-privilege hardening is still needed before calling this properly production-ready
