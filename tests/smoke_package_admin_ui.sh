...
grep -q '^Environment=MFA_SIDECAR_INSTALL_DIR=/opt/yunohost/mfa_sidecar$' "$SERVICE"
grep -q 'subprocess.run(\["sudo", apply_helper, DEFAULT_INSTALL_DIR\], check=True)' "$ROOT_DIR/package-base/sources/admin_ui_app.py"
